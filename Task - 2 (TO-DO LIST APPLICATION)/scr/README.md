...
- [**Queries**](#queries)
  - [*ListTasksByList*](#listtasksbylist)
- [**Mutations**](#mutations)
...
## ListTasksByList
You can execute the `ListTasksByList` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
listTasksByList(vars: ListTasksByListVariables, options?: ExecuteQueryOptions): QueryPromise<ListTasksByListData, ListTasksByListVariables>;
...
```typescript
listTasksByList(dc: DataConnect, vars: ListTasksByListVariables, options?: ExecuteQueryOptions): QueryPromise<ListTasksByListData, ListTasksByListVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the listTasksByListRef:
```typescript
const name = listTasksByListRef.operationName;
console.log(name);
```

### Variables
The `ListTasksByList` query requires an argument of type `ListTasksByListVariables`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
...
```
### Return Type
Recall that executing the `ListTasksByList` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `ListTasksByListData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `ListTasksByList`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, listTasksByList, ListTasksByListVariables } from '@dataconnect/generated';

// The `ListTasksByList` query requires an argument of type `ListTasksByListVariables`:
const listTasksByListVars: ListTasksByListVariables = {
  listId: ..., 
};

// Call the `listTasksByList()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await listTasksByList(listTasksByListVars);
// Variables can be defined inline as well.
const { data } = await listTasksByList({ listId: ..., });
...
listTasksByList(listTasksByListVars).then((response) => {
  const data = response.data;
  console.log(data.tasks);
});
```

### Using `ListTasksByList`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, listTasksByListRef, ListTasksByListVariables } from '@dataconnect/generated';
...
// Call the `listTasksByListRef()` function to get a reference to the query.
const ref = listTasksByListRef(listTasksByListVars);
// Variables can be defined inline as well.
const ref = listTasksByListRef({ listId: ..., });

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = listTasksByListRef(dataConnect, listTasksByListVars);
...
// Variables can be defined inline as well.
const { data } = await createTask({ listId: ..., title: ..., description: ..., });
...
// Variables can be defined inline as well.
const ref = createTaskRef({ listId: ..., title: ..., description: ..., });
...
// Variables can be defined inline as well.
const { data } = await updateTaskStatus({ id: ..., isCompleted: ..., });
...
// Variables can be defined inline as well.
const ref = updateTaskStatusRef({ id: ..., isCompleted: ..., });
